...
class TestSerialNumber(unittest.TestCase):
    ...
    def test_from_bytes(self) -> None:
        ...
        with self.assertRaises(ValueError):
            SerialNumber.from_bytes(b"\x00\x01\x02\x03\x04\x05\x06\x07")
    ...
    def test_from_str(self) -> None:
        ...
        with self.subTest(value=b"1-v1-1"):
            with self.assertRaises(TypeError):
                SerialNumber.from_str(b"1-v1-1")

        # Too many separators
        with self.subTest(value="0-0-0-0"):
            with self.assertRaises(ValueError):
                SerialNumber.from_str("0-0-0-0")

        # Bad separator given
        with self.subTest(value="0.v0.0"):
            with self.assertRaises(ValueError):
                SerialNumber.from_str("0.v0.0")

        # Invalid pcb_id
        with self.subTest(value="one-v1-1"):
            with self.assertRaises(ValueError):
                SerialNumber.from_str("one-v1-1")

        # Invalid version
        with self.subTest(value="1-version1-1"):
            with self.assertRaises(ValueError):
                SerialNumber.from_str("1-version1-1")

        # Invalid instance_id
        with self.subTest(value="1-v1-one"):
            with self.assertRaises(ValueError):
                SerialNumber.from_str("1-v1-one")

        # Test a couple valid strings
        self.assertEqual(SerialNumber(1, 1, 1), SerialNumber.from_str("1-1-1"))
        self.assertEqual(SerialNumber(1, 1, 1), SerialNumber.from_str("001-v1-00001"))
    ...
    def test_to_str(self) -> None:
        ...
        test_cases = [
            SerialNumber(1, 1, 1),
            SerialNumber(23, 0, 256),
        ]
        for sn in test_cases:
            with self.subTest(sn=sn):
                self.assertEqual(sn, SerialNumber.from_str(str(sn)))
